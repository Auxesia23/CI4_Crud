...
class Home extends BaseController
{
    ...
    public function downloadCsv()
    {
        ...
        // Prepare to filter and format data
        $filteredData = [];

        foreach ($bukuData as $row) {
            $filteredData[] = [
                'id_buku' => $row['id_buku'], // Assuming this is the correct key from your model
                'kategori' => $row['kategori'], // Adjust according to your model keys
                'nama_buku' => $row['nama_buku'],
                'harga' => $row['harga'],
                'stok' => $row['stok'],
                'penerbit' => $row['nama'], // If you join data, ensure this key exists
            ];
        }

        // Define CSV headers
        $filename = "daftar_buku_" . date('Ymd') . ".csv";
        ...
        $header = ['ID Buku', 'Kategori', 'Nama Buku', 'Harga', 'Stok', 'Penerbit'];
        fputcsv($file, $header);
